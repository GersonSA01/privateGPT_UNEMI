...
def _create_metadata_filters(
    ...
    """
    Create metadata filters for both document IDs and role-based filtering.
    Uses getattr to safely access 'access_roles' to avoid crashes.
    """
    filters_list = []
    
    if context_filter is not None:
        # 1. Filter by document IDs (Standard PrivateGPT behavior)
        if context_filter.docs_ids:
            for doc_id in context_filter.docs_ids:
                filters_list.append(MetadataFilter(key="doc_id", value=doc_id))
        
        # 2. Filter by user roles (SAFE ACCESS FIX)
        # Usamos getattr por si el objeto ContextFilter no tiene el campo 'access_roles'
        access_roles = getattr(context_filter, "access_roles", None)
        
        if access_roles:
            for rol in access_roles:
                filters_list.append(MetadataFilter(key="role", value=rol))

    # OR condition: documents that match ANY of the filters (ID match OR Role match)
...
class VectorStoreComponent:
    ...
    def get_retriever(
        ...
        # Generate combined filters (IDs + Roles)
        custom_filters = _create_metadata_filters(context_filter) if context_filter else None

        return VectorIndexRetriever(
            index=index,
            similarity_top_k=similarity_top_k,
            doc_ids=context_filter.docs_ids if context_filter else None,
            # Apply filters to ALL vector stores (including Qdrant)
